...
void senzorPody()
{
  ...
  Blynk.virtualWrite(V11, senzorPodyPercento);  // Zapisanie percentuálnej hodnoty do virtuálneho pinu V11
...
void snimanieBaterie() // Funkcia snímania stavu baterie
{
  ...
  Blynk.virtualWrite(V12, senzorBateriaPercento); // Zapisanie percentuálnej hodnoty do virtuálneho pinu V12
...
void DHTSenzor() {
  ...
  Serial.print(F("°C "));
  Blynk.virtualWrite(V10, teplota); // Zapisanie nameranej hodnoty teploty do virtuálneho pinu V10
}


// *** Status senzora pre BLYNK *** //
void statusSenzora(){
  Blynk.virtualWrite(V13, 1); // odoslanie hodnoty 1 do virtuálneho pinu V13 v Blynk aplikácii
}
