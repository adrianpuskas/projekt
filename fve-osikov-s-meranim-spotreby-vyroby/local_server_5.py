...
# Kľúčové metriky na hlavnej stránke (Home)
KEY_METRICS = {
...
# Detailné info o všetkých hodnotách (pre sekciu Detail)
VPINS = {
    # PV1
...
<head>
  ...
  <style>
    ...
    CSS/* Kompaktný pill design pre BMS tlačidlá */
    .bms-pill-btn {
      height: 100px; /* menšie ako predtým */
    ...
    .bms-pill-btn i {
      font-size: 2.5rem; /* veľká ikona */
      margin-bottom: 8px;
    }
    .bms-pill-btn .main-text {
      font-size: 1.3rem;
      font-weight: bold;
    }
    .bms-pill-btn .sub-text {
      font-size: 1rem;
      opacity: 0.9;
    }
...
<body>
    ...
    <div class="container-fluid">
      ...
      <div class="collapse navbar-collapse" id="navbarNav">
        <ul class="navbar-nav me-auto">
          ...
          <li class="nav-item"><a class="nav-link" onclick="showSection('topenie')">Topenie</a></li>
        </ul>
  ...
  <div class="container">
    ...
    <div id="grafy" class="section">
      ...
      <!-- Tabuľka článkov – ako v JK appke -->
      <div class="card p-3 shadow">
        <h4 class="text-center text-primary mb-3">Napätia článkov</h4>
        <div class="row g-2" id="bms-cells">
          <!-- Sem sa dynamicky vložia karty článkov -->
        </div>
      </div>

      <!-- Ovládanie BMS – finálna verzia s viditeľnými ikonami -->
      <div class="row justify-content-center mt-5">
        <div class="col-12 col-md-10 col-lg-8">
          ...
          <div class="row g-3 mb-4 justify-content-center">
            <div class="col-5">
              <button class="btn btn-success w-100 py-3 rounded-pill shadow bms-btn-hover d-flex flex-column align-items-center justify-content-center"
            ...
            <div class="col-5">
              <button class="btn btn-danger w-100 py-3 rounded-pill shadow bms-btn-hover d-flex flex-column align-items-center justify-content-center"
          ...
          <div class="row g-3 justify-content-center">
            <div class="col-5">
              <button class="btn btn-info w-100 py-3 rounded-pill shadow bms-btn-hover d-flex flex-column align-items-center justify-content-center"
  ...
  <script>
    ...
    function updateInfoBar() {
      // NOVÉ: vnútorná teplota a vlhkosť
    ...
    function renderTopenie() {
      const c = document.getElementById('topenie-content');
      c.innerHTML = '';
      const topenieKeys = [
        "topenie_water_temp", "topenie_boiler_temp", "topenie_smoke_temp", "topenie_door_position"
      ];
      topenieKeys.forEach(key => {
        if (data[key] !== undefined) {
          const meta = info[key] || [key, "", "", "fas fa-circle"];
          c.innerHTML += createCard(key, meta[0], data[key], meta[2], meta[3]);
        }
      });
      // Ovládanie
      c.innerHTML += `
        <div class="col-12 mb-4">
          <div class="detail-group">
            <h4>Ovládanie</h4>
            <div class="row justify-content-center">
              <button class="btn btn-primary" onclick="sendCmd('topenie_manual_mode', data['topenie_manual_mode'] == 1 ? 0 : 1)">Manuálny režim: ${data['topenie_manual_mode'] || 0}</button>
              <button class="btn btn-primary" onclick="sendCmd('topenie_pump', data['topenie_pump'] == 1 ? 0 : 1)">Čerpadlo: ${data['topenie_pump'] || 0}</button>
              <button class="btn btn-primary" onclick="sendCmd('topenie_valve_tuv', data['topenie_valve_tuv'] == 1 ? 0 : 1)">Ventil TUV: ${data['topenie_valve_tuv'] || 0}</button>
              <button class="btn btn-primary" onclick="sendCmd('topenie_valve_radiator', data['topenie_valve_radiator'] == 1 ? 0 : 1)">Ventil radiatorov: ${data['topenie_valve_radiator'] || 0}</button>
              <button class="btn btn-primary" onclick="sendCmd('topenie_priority', data['topenie_priority'] == 1 ? 0 : 1)">Priorita: ${data['topenie_priority'] || 0}</button>
              <button class="btn btn-primary" onclick="sendCmd('topenie_door_close_button', data['topenie_door_close_button'] == 1 ? 0 : 1)">Zatvoriť dvierka: ${data['topenie_door_close_button'] || 0}</button>
              <button class="btn btn-primary" onclick="sendCmd('topenie_init', 1)">Inicializácia</button>
              // Pridaj sliders (použi <input type="range" oninput="sendCmd('key', this.value)">)
              <input type="range" min="0" max="100" value="${data['topenie_pump_min'] || 0}" oninput="sendCmd('topenie_pump_min', this.value)" />
              // Podobne pre ostatné sliders
            </div>
          </div>
        </div>
      `;
    ...
    function renderBMSPage() {
      ...
      const minV     = data['V235'] || null;
      const maxCell  = data['V236'] || null;
      const maxV     = data['V237'] || null;
      ...
      // Maximálne predpokladáme 24 článkov (stačí na väčšinu JK BMS)
      for (let i = 0; i < 24; i++) {
        const pin = `V${240 + i}`;
