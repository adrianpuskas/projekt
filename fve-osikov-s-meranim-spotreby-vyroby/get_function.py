...
def nastavenia_menica():
    ...
    try:
        ...
        pass  # ak QFLAG zlyhá, flags ostanú prázdne
...
def nastaveneData():
    ...
    try:
        ...
        try:
            ...
            # v91 = Max AC charging current (index 13)
            settings["v91"] = int(decoded_values[13].lstrip('0') or 0)
            print(f"\tMax AC charging current: {settings['v91']} A", flush=True)

            # v92 = Max charging current (index 14)
            settings["v92"] = int(decoded_values[14].lstrip('0') or 0)
            print(f"\tMax charging current: {settings['v92']} A", flush=True)

            # v93 = Input voltage range (index 15)
            settings["v93"] = int(decoded_values[15])
            print(f"\tInput voltage range: {'Appliance' if settings['v93'] == 0 else 'UPS'}", flush=True)

            # v94 = Output source priority (index 16)
            settings["v94"] = int(decoded_values[16])
            print(f"\tOutput source priority: {settings['v94']}", flush=True)

            # v95 = Charger source priority (index 17)
            settings["v95"] = int(decoded_values[17])
            print(f"\tCharger source priority: {settings['v95']}", flush=True)

            # v86 = Battery re-discharge voltage (index 22)
            ...
            if response2:
                ...
                if len(decoded2) > 1:
                    settings["v98"] = float(decoded2[1])
                    print(f"\tAC output frequency: {settings['v98']} Hz", flush=True)

        except Exception as e:
            print(f"\tChyba pri dekódovaní QPIRI: {e}", flush=True)

    except Exception as e:
        print(f"\tChyba pri QPIRI: {e}", flush=True)

    return settings  # vždy vráti dict (aj s defaultmi pri chybe)
