...
def main_process():
    ...
    while not stop_event.is_set():
        print("Starting energy_meter.py...")
        ...
        proc = subprocess.Popen(["C:/Users/adria/AppData/Local/Programs/Python/Python312/python", "energy_meter.py"],
                                stdin=subprocess.PIPE, env=env)

        while not stop_event.is_set():
            if proc.poll() is not None:
                print(f"energy_meter.py exited with code {proc.returncode}")
                break
            ...
            try:
                ...
                print(f"Error occurred: {e}, killing energy_meter.py...")
                proc.kill()
                break
