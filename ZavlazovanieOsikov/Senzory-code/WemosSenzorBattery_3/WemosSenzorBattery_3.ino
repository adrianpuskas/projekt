...
void senzorPody()
{
  ...
  Blynk.virtualWrite(V31, senzorPodyPercento);  // Zapisanie percentuálnej hodnoty do virtuálneho pinu V31
...
void snimanieBaterie() // Funkcia snímania stavu baterie
{
  ...
  Blynk.virtualWrite(V32, senzorBateriaPercento); // Zapisanie percentuálnej hodnoty do virtuálneho pinu V32
...
void DHTSenzor() {
  ...
  Serial.print(F("°C "));
  Blynk.virtualWrite(V30, teplota); // Zapisanie nameranej hodnoty teploty do virtuálneho pinu V30
}


// *** Status senzora pre BLYNK *** //
void statusSenzora(){
  Blynk.virtualWrite(V33, 1); // odoslanie hodnoty 1 do virtuálneho pinu V33 v Blynk aplikácii
}
