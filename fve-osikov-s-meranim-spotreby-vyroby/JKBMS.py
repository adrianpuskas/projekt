...
readCommand = ('4E 57 00 13 00 00 00 00 06 03 00 00 00 00 00 00 68 00 00 01 29')
offChargeCommand = ('4E 57 00 14 00 00 00 00 02 03 02 ab 00 00 00 00 00 68 00 00 01 d3')
onChargeCommand = ('4E 57 00 14 00 00 00 00 02 03 02 ab 01 00 00 00 00 68 00 00 01 d4')
offDischargeCommand = ('4E 57 00 14 00 00 00 00 02 03 02 ac 00 00 00 00 00 68 00 00 01 d4')
onDischargeCommand = ('4E 57 00 14 00 00 00 00 02 03 02 ac 01 00 00 00 00 68 00 00 01 d5')
...
\n"""
                        ...
                        # === NOVÉ: Posielanie jednotlivých hodnôt na samostatné piny (pre local_server) ===

                        # Celkové hodnoty
                        blynk_write(230, f"{voltage:.2f}")          # Celkové napätie     → V130
                        blynk_write(231, f"{current:.2f}")          # Prúd                → V131
                        blynk_write(232, str(capacity))             # SOC %               → V132
                        blynk_write(233, f"{delta_u:.3f}")          # Delta U             → V133
                        blynk_write(234, str(min_index))            # Najnižší článok č.  → V134
                        blynk_write(235, f"{min_voltage:.3f}")      # Najnižšie napätie   → V135
                        blynk_write(236, str(max_index))            # Najvyšší článok č.  → V136
                        blynk_write(237, f"{max_voltage:.3f}")      # Najvyššie napätie   → V137
                        blynk_write(238, str(temp_1))               # Teplota 1           → V138
                        blynk_write(239, str(temp_2))               # Teplota 2           → V139

                        # Napätia všetkých článkov – jednotlivo na V140 až V140+cellcount-1
                        for i in range(cellcount):
                            pin = 240 + i                           # V140 = Cell1, V141 = Cell2, atď.
                            blynk_write(pin, f"{cell_voltages[i]:.3f}")
...
def sendBMSCommandWithRetry(cmd_string, max_retries=3, delay=0.2):
    """
    Všeobecné odoslanie BMS príkazu s retry mechanizmom.
    Zatiaľ nekontroluje odpoveď – len pošle príkaz.
    """
    for attempt in range(max_retries):
        try:
            if bms is None or not bms.is_open:
                print(" BMS port nie je otvorený.")
                return False

            sendBMSCommand(cmd_string)
            print(f" BMS príkaz odoslaný (pokus {attempt + 1}/{max_retries})")

            time.sleep(delay)

            # ?? zatiaľ odpoveď NEVYHODNOCUJEME
            # neskôr tu môžeš:
            # - čítať ACK
            # - porovnávať stav charge/discharge
            # - volať readBMS()

            return True

        except Exception as e:
            print(f" Chyba pri odoslaní BMS príkazu (pokus {attempt + 1}/{max_retries}):\n\t{e}")
            time.sleep(delay)

    print(" BMS príkaz sa nepodarilo odoslať.")
    return False


def offChargeCommandBMS(max_retries=3):
    return sendBMSCommandWithRetry(offChargeCommand, max_retries)

def onChargeCommandBMS(max_retries=3):
    return sendBMSCommandWithRetry(onChargeCommand, max_retries)

def offDischargeCommandBMS(max_retries=3):
    return sendBMSCommandWithRetry(offDischargeCommand, max_retries)

def onDischargeCommandBMS(max_retries=3):
    return sendBMSCommandWithRetry(onDischargeCommand, max_retries)
