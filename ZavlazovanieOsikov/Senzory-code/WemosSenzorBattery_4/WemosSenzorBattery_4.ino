i #define BLYNK_TEMPLATE_ID           "TMPLblK60Pj-"  // definícia Blynk šablóny ID
...
void senzorPody()
{
  ...
  Blynk.virtualWrite(V41, senzorPodyPercento);  // Zapisanie percentuálnej hodnoty do virtuálneho pinu V41
...
void snimanieBaterie() // Funkcia snímania stavu baterie
{
  ...
  Blynk.virtualWrite(V42, senzorBateriaPercento); // Zapisanie percentuálnej hodnoty do virtuálneho pinu V42
...
void DHTSenzor() {
  ...
  Serial.print(F("°C "));
  Blynk.virtualWrite(V40, teplota); // Zapisanie nameranej hodnoty teploty do virtuálneho pinu V40
}


// *** Status senzora pre BLYNK *** //
void statusSenzora(){
  Blynk.virtualWrite(V43, 1); // odoslanie hodnoty 1 do virtuálneho pinu V43 v Blynk aplikácii
}
