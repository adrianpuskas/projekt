...
def get_shared_data():
    try:
        with open("zdielane_data_pre_TUV.json", "r") as f:
            data = json.load(f)
        return data
    except:
        return {}
...
vykon_panely = data.get("pv_input_power", 0)
vykon_zataz = data.get("output_load_power", 0)
vykon_bateria = data.get("battery_power", 0)
kapacita_bateria = data.get("battery_capacity", 0)
print(napatie_panely, teplota_menica, vykon_panely, vykon_zataz, vykon_bateria, kapacita_bateria)
"""
...
def nacitanie_hodnôt_automatizacie():
    ...
    global BAT_CAP_OFF
    BAT_CAP_OFF = int(read(AutomatizaciaToken, 7))      # - Kedy sa vypne, ak klesne pod túto hodnotu
    print (f" | BAT_CAP_OFF - {BAT_CAP_OFF}%")
            ...
            if napatie_panely > 50 and teplota_menica > 1:
                ...
                if read_inverter_mode == "Battery":
                    ...
                    if stav_ohrevu == 0:
                        ...
                        elif priorita_FVE == 0 and vykon_zataz < LIMIT_ZAP_ZATAZ and kapacita_bateria >= BAT_CAP_ON and stav_ohrevu == 0 and moze_sa_zapnut_ohrev():
                        ...
                        elif priorita_FVE == 1 and vykon_zataz < LIMIT_ZAP_ZATAZ and stav_ohrevu == 0 and moze_sa_zapnut_ohrev():
                            write(*ohrev_vypinac, 1)
                            write(*nastaveny_vykon_ohrevu, MIN_VYKON_START)
                            #ohrev_nedavno_vypnuty = False
                        ...
                        if priorita_FVE == 0 and kapacita_bateria < BAT_CAP_ON: ## 0 - bateria / 1 - TUV
                            ...
                            """    
                            if BAT_MAX_DISCHARGE > vykon_bateria > BAT_SAFE_RANGE:
                                new_power = min(vykon_ohrevu + VYKON_KROK, MAX_VYKON)
                                write(*nastaveny_vykon_ohrevu, new_power)
                                print(f"Zvyšujem výkon ohrevu na {new_power}%")
                                time.sleep(1.5)
                            """
                            ...
                            """    
                            elif vykon_bateria < BAT_MAX_DISCHARGE and vykon_ohrevu > MIN_VYKON:
                                new_power = max(vykon_ohrevu - VYKON_KROK, MIN_VYKON)
                                write(*nastaveny_vykon_ohrevu, new_power)
                                print(f"Znižujem ohrev na {new_power}%")
                            """    
